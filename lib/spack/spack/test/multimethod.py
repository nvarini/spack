...
def test_no_version_match(mock_packages):
    pkg = spack.repo.path().get('multimethod@2.0')
    with pytest.raises(NoSuchMethodError):
        pkg.no_version_2()


def test_one_version_match(mock_packages):
    pkg = spack.repo.path().get('multimethod@1.0')
    assert pkg.no_version_2() == 1

    pkg = spack.repo.path().get('multimethod@3.0')
    assert pkg.no_version_2() == 3

    pkg = spack.repo.path().get('multimethod@4.0')
...
def test_version_overlap(mock_packages):
    ...
    pkg = spack.repo.path().get('multimethod@5.0')
    assert pkg.version_overlap() == 2


def test_mpi_version(mock_packages):
    pkg = spack.repo.path().get('multimethod^mpich@3.0.4')
    assert pkg.mpi_version() == 3

    pkg = spack.repo.path().get('multimethod^mpich2@1.2')
    assert pkg.mpi_version() == 2

    pkg = spack.repo.path().get('multimethod^mpich@1.0')
    assert pkg.mpi_version() == 1


def test_undefined_mpi_version(mock_packages):
    pkg = spack.repo.path().get('multimethod^mpich@0.4')
    assert pkg.mpi_version() == 1

    pkg = spack.repo.path().get('multimethod^mpich@1.4')
    assert pkg.mpi_version() == 1


def test_default_works(mock_packages):
    pkg = spack.repo.path().get('multimethod%gcc')
    assert pkg.has_a_default() == 'gcc'

    pkg = spack.repo.path().get('multimethod%intel')
    assert pkg.has_a_default() == 'intel'

    pkg = spack.repo.path().get('multimethod%pgi')
...
def test_target_match(mock_packages):
    ...
    for target in targets[:-1]:
        pkg = spack.repo.path().get('multimethod target=' + target.name)
        assert pkg.different_by_target() == target.name

    pkg = spack.repo.path().get('multimethod target=' + targets[-1].name)
    if len(targets) == 1:
...
def test_dependency_match(mock_packages):
    pkg = spack.repo.path().get('multimethod^zmpi')
    assert pkg.different_by_dep() == 'zmpi'

    pkg = spack.repo.path().get('multimethod^mpich')
    ...
    pkg = spack.repo.path().get('multimethod^foobar')
    assert pkg.different_by_dep() == 'mpich'


def test_virtual_dep_match(mock_packages):
    pkg = spack.repo.path().get('multimethod^mpich2')
...
def test_multimethod_with_base_class(mock_packages):
    pkg = spack.repo.path().get('multimethod@3')
    assert pkg.base_method() == "subclass_method"

    pkg = spack.repo.path().get('multimethod@1')
