...
class PackageBase(with_metaclass(PackageMeta, object)):
    ...
    **Package Lifecycle**
        ...
        for i, name in enumerate(self.dependencies):
            ...
            for dep_name in dep_names:
                if dep_name not in visited:
                    ...
                    if transitive:
                        pkg = spack.repo.path().get(dep_name)
    ...
    def test_check(self):
        ...
        Normally returns ``True``, but subclasses should return ``None``
        to skip the real install and only do tests.
        """
        return True
